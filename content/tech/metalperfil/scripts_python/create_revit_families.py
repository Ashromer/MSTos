...
# Asegurar que el directorio del proyecto está en el path para importar extract_correct
...
import extract_correct

# Automatización: Ejecutar extracción antes de procesar
print("Iniciando extracción CAD...")
extract_correct.extract()
...
def normalize(vertices):
    """Alinea el perfil en planta según las directrices de Metalperfil (Fase 3)"""
    if not vertices: return []
    xs = [v[0] for v in vertices]
    min_x, max_x = min(xs), max(xs)
    cx = (min_x + max_x) / 2.0
    
    # Rango horizontal del 15% al 85%
    ancho = max_x - min_x
    limite_izq = min_x + 0.15 * ancho
    limite_der = min_x + 0.85 * ancho
    
    # Buscar el Y mínimo (puntos más bajos de apoyo) en ese rango central
    puntos_centrales_y = [v[1] for v in vertices if limite_izq <= v[0] <= limite_der]
    if not puntos_centrales_y:
        puntos_centrales_y = [v[1] for v in vertices]
        
    min_y_range = min(puntos_centrales_y)
    
    # Trasladar toda la geometría en bloque: centrar en X y alinear base de apoyo a Y=0
    return [(v[0] - cx, v[1] - min_y_range, v[2] if len(v) > 2 else 0.0) for v in vertices]
    ...
    for i in range(1, len(pts_mm)):
        ...
        if last_pt.DistanceTo(curr_pt) > tol:
            ...
            if abs(bulge) < 1e-5:
                ...
                # Se invierte el signo del bulge para corregir la dirección en AcerOnda
                corrected_bulge = -bulge 
                x1, y1 = pts_mm[i-1][0], pts_mm[i-1][1]
                x2, y2 = pts_mm[i][0], pts_mm[i][1]
                mx = (x1 + x2) / 2.0 - corrected_bulge * (y2 - y1) / 2.0
                my = (y1 + y2) / 2.0 + corrected_bulge * (x2 - x1) / 2.0
